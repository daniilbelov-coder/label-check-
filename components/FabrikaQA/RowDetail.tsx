...
export function RowDetail({ jobId, rowId, row }: { jobId: string; rowId: string; row: FabrikaRow }) {
  ...
  if (full.status === 'error') return <div className="text-red-700">Ошибка: {full.error}</div>;
  if (full.status === 'no-spec') return <div className="text-amber-700">Нет подходящей колонки в XLSX для этого PDF.</div>;
  if (!full.mainMd) return <div className="text-neutral-500">Загрузка…</div>;
          ...
          <ul className="mt-2 space-y-2">
            {full.signResults.map((s, i) => (
              <li key={i} className="rounded bg-white p-2 text-xs">
                ...
                {s.error ? <div className="text-red-700">{s.error}</div> : <pre className="whitespace-pre-wrap">{s.raw}</pre>}
              </li>
            ))}
          </ul>
        </details>
